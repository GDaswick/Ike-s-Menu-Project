...
#Things to add:
#**Split the actual scraping of data and the program into two separate scripts; make the first file output text files**
#1. Add a better way to keep track of the ingredients and stuff (within the code itself)
#2. Add categories  for ingredients: meats, vegetables, sauces, etc.
#3. Make this work for other Ike's locations (and make it so that uncategorized new ingredients can be input into new text files
#4. Make going back in the iterations better
...
#Begin gathering the menu information
driver.get("https://order.myguestaccount.com/menu/60902b7495b701063f8b4597?from_locations=1&appid=5ffe1d20adb0d57c1c96ada1")#navigate to site
driver.find_element(by = By.XPATH, value = "//*[@id='start-order-type']/div[1]/div[2]/button[1]").click()
try: #This button only exists when they're open
    driver.find_element(by = By.XPATH, value = "//*[@id='start-order-time-info']/div[2]/button[1]").click()
except ElementClickInterceptedException:
    full_text += "Ike's is currently closed!"
...
items = driver.find_elements(by = By.CLASS_NAME, value = "item-info")
for i in items:
    all_names_v1.append(i.find_element(by = By.TAG_NAME, value = 'h3').text.title())
    all_desc_v1.append(i.find_element(by = By.CLASS_NAME, value = "item-description").text.split('.')[0].title())
...
#process all descriptions into a list of ingredients; just removing duplicates, formatting, etc.
all_ing = []
count = 0
...
#Begin begin the interactive portion of the program (GUI, etc.)
